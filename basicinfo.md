...
### 공암산성 (선적) — 라즈베리파이 저울 서버
| 항목 | 값 |
|------|---|
| **장비** | Raspberry Pi |
| **내부 IP** | `192.168.0.250` |
| **OS** | Raspbian (Linux) |
| **SSH 계정** | `pi` / `bluenet2002` |
| **서비스** | `custom-scale.service` (systemd, enabled) |
| **소스 파일** | `/home/pi/scale_service.py` (V2.2) |
| **설정 파일** | `/home/pi/scale_config.json` |
| **웹 UI** | `http://192.168.0.250` (Port 80, 3구간 공차 설정) |
| **JSON TCP** | Port 13450 (키오스크/DISPATCH 연동) |
| **리부팅** | 매일 12시 자동 리부팅 |

#### RS-232C 시리얼 포트 매핑 (2026-03-29 현장 검증)
| 물리포트 | 리눅스 장치명 | 용도 | Baud | 프로토콜 |
|---|---|---|---|---|
| Port 4 | `/dev/ttyUSB0` 또는 `/dev/tty232_0` | 입력(저울 FS-4300) | 9600 | ASCII |
| Port 5 | `/dev/tty232_1` | 출력(전광판 FINE UER 1.9) | 9600 | CAS 호환 |

#### 전광판 통신 규격
- **모델**: FINE UER 1.9
- **부팅 시퀀스**: `fine` → `uer 1` → `.9` → `0`(대기)
- **프로토콜**: CAS 호환 (`ST,GS,{부호}{무게6자리} kg\r\n`)
- **예시**: `ST,GS,+  2115 kg\r\n` (22바이트)
- **통신**: RS-232C 9600 Baud, 8N1, No Flow Control

#### 장애 이력
- **2026-03-29**: observer.out 프로세스 충돌 + 통신 규격·속도 불일치 → 전광판 '0' 블링킹. observer.out 영구 삭제, CAS 규격 9600 Baud 하드코딩, V2.2 마스터본 배포로 해결.
...
| 서비스 | 키 | 환경변수/비고 |
|--------|---|------------|
| **Gemini API** | `AIzaSy`+`Cw97_GAmLZ3NYIK`+`4QdvbK8cpako8f7kTw` (Github스캐너 방지용 분할 표기) | `GEMINI_API_KEY` / `GOOGLE_API_KEY` |
...
*최종 업데이트: 안티그래비티 — 2026-03-29 23:18 KST — 라즈베리파이 저울 서버(RS-232C 포트 매핑, FINE 전광판 통신 규격, 장애 이력) 추가*
